// Play-by-play simulation for 3 innings printing engine internals (FP, H_raw, etc.)
// Mirrors logic from src/engine/baseball.ts without imports. Deterministic via seeded PRNG.
...
function computeEventProbsWithDebug(batter, oppPitch) {
  ...
  let reach_def = null, reach_blend = null, scale = 1;
  if (typeof oppPitch.WHIP === "number" && isFinite(oppPitch.WHIP) && oppPitch.WHIP > 0) {
    reach_def = reachFromWHIP(oppPitch.WHIP);
    reach_blend = REACH_OFF_WEIGHT * reach_off + REACH_DEF_WEIGHT * reach_def;
    scale = reach_off > 1e-9 ? reach_blend / reach_off : 1;
  ...
  const probs = { OUT: pOUT, BB: P_BB, HBP: P_HBP, "1B": p1B, "2B": p2B, "3B": p3B, HR: pHR };
  const debug = { AVG, OBP, SLG, ERA, FP, H_raw, OBP_eff, BB_raw, reach_off, reach_def, reach_blend, scale, H, BB, P_HBP, P_BB, H_base, basesPerHit, w1, w2, w3, wHR, p1B, p2B, p3B, pHR, pOUT };
  return { probs, debug };
}

function eventProbsForHalfWithDebug(half, home, away) {
  const batting = half === "top" ? away.bat : home.bat;
  const pitching = half === "top" ? home.pitch : away.pitch;
  return computeEventProbsWithDebug(batting, pitching);
}
...
function simulateWithTrace(home, away, rules = DEFAULT_RULES, seed = 12345) {
  ...
  while (!gs.status.over && plays++ < 10000) {
    const key = `${gs.inning}-${gs.half}`;
    const { probs, debug } = eventProbsForHalfWithDebug(gs.half, home, away);
    if (key !== lastHalfKey) {
      const who = gs.half === "top" ? "Away batea vs Home pitcher" : "Home batea vs Away pitcher";
      log.push(`\n=== Inning ${gs.inning} ${gs.half === "top" ? "alta" : "baja"} | ${who} ===`);
      log.push(`Inputs: AVG=${fmt(debug.AVG)}, OBP=${fmt(debug.OBP)}, SLG=${fmt(debug.SLG)}, ERA=${fmt(debug.ERA)}`);
      log.push(`FP=${fmt(debug.FP)}  H_raw=${fmt(debug.H_raw)}  OBP_eff=${fmt(debug.OBP_eff)}  BB_raw=${fmt(debug.BB_raw)}  reach_off=${fmt(debug.reach_off)}`);
      if (debug.reach_def != null) {
        log.push(`reach_def=${fmt(debug.reach_def)}  blend=${fmt(debug.reach_blend)}  scale=${fmt(debug.scale)}`);
      }
      log.push(`H=${fmt(debug.H)}  BB=${fmt(debug.BB)}  P_BB=${fmt(debug.P_BB)}  P_HBP=${fmt(debug.P_HBP)}`);
      log.push(`H_base=${fmt(debug.H_base)}  basesPerHit=${fmt(debug.basesPerHit)}  mix: w1=${fmt(debug.w1)} w2=${fmt(debug.w2)} w3=${fmt(debug.w3)} wHR=${fmt(debug.wHR)}`);
      log.push(`Probs: OUT=${fmt(probs.OUT)} BB=${fmt(probs.BB)} HBP=${fmt(probs.HBP)} 1B=${fmt(probs["1B"])} 2B=${fmt(probs["2B"])} 3B=${fmt(probs["3B"])} HR=${fmt(probs.HR)}`);
      lastHalfKey = key;
    }
...
// Run and print trace with internals
