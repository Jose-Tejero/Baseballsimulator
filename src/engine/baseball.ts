// Tipos y motor (Nivel 2: AVG·OBP·SLG·ERA)
...
type GameEndReason =
  | "regulation" // terminó en 9 entradas con ganador / regla general
  | "walkoff" // home toma la ventaja definitiva en la baja
  | "mercy" // regla de nocaut
  | "maxInnings" // alcanzó tope de entradas
...
export type Rules = {
  regulationInnings: number; // Entradas “reglamentarias” (normalmente 9; en ligas menores/softball puede ser 7)
  allowTies: boolean; // ¿Se permite terminar empatado?
  enableExtraInnings: boolean; // ¿Se habilitan entradas extra si hay empate tras las reglamentarias?
  maxInnings: number | null; // Tope duro de entradas (p.ej. 12). Null = sin tope.
  walkoff: boolean; // ¿Se permite “walk-off” en la baja cuando el home toma ventaja definitiva?
  mercyDiff?: number; // Diferencia para “mercy rule” (nocaut), p.ej. 10
  mercyInning?: number; // A partir de qué entrada aplica la mercy, p.ej. 7
};
...
/** Normaliza reglas para evitar configuración sin salida:
 *  - Si NO hay extras y NO se permiten empates → habilita extras.
 */
...
// --- Helpers estocásticos (Paso 1 realismo) ---
function rnd() {
  return Math.random();
}
...
// Nótese que esto es un modelo heurístico y calibrable.
...
function dpProbByOuts(outs: number) {
  // Prob. de DP sólo aplicable a GB con 1B ocupado y <2 outs
  return outs === 0 ? 0.12 : outs === 1 ? 0.14 : 0;
}

function sfProbByOuts(outs: number) {
  // Prob. de SF sólo aplicable a FB con 3B ocupado y <2 outs
  return outs === 0 ? 0.04 : outs === 1 ? 0.08 : 0;
}

/** Avance estocástico según estado base/outs y tipo de hit. */
function advanceRunnersStochastic(gs: GameState, basesToAdvance: number) {
  ...
  // Tablas heurísticas (aprox MLB) dependientes de outs
  const P = {
    single: {
  ...
  if (basesToAdvance === 1) {
    ...
    // Bateador a 1B (si bases estaban llenas y nadie anotó de 3B, puede empujar carrera)
    if (b0.first && b0.second && b0.third && !nb.third) {
      // bases llenas & sencillo con retención: empuja 1
      runs++;
      nb.third = true; // 2B->3B
    ...
    // HR (aquí no debería entrar; HR se maneja aparte)
...
function reachFromWHIP(whip: number) {
  // WHIP típico MLB ~0.8–1.6 → reach ~0.21–0.35
  const r = whip / (3 + whip);
  return clamp(r, 0.12, 0.48); // límites de seguridad
}

// Peso del “lado bateador” vs “lado pitcher”
...
function checkEndOfHalf(gs: GameState) {
  ...
  // Estamos en 'bottom' → terminó la ALTA del inning actual (gs.inning).
  if (gs.half === "bottom") {
  ...
  // Estamos en 'top' → terminó la BAJA del inning anterior (gs.inning - 1).
  ...
  if (afterRegComplete) {
    ...
    // Empate después de 9 completas:
    if (!r.enableExtraInnings) {
      // Con normalización, si no hay extras entonces allowTies es true.
      gs.status = { over: true, winner: "tie", reason: "tieAllowed" };
      return;
    }
    // Revisar límite de entradas extra, si lo hay:
...
function forceAdvanceOneBase(gs: GameState) {
  ...
  if (b.first && b.second && b.third) {
    // Bases llenas → entra 1
    scoreRun(gs, 1);
    // Desplaza (se mantienen llenas: el anotador sale, los demás avanzan uno)
    b.third = true;
    b.second = true;
    b.first = true;
    return;
...
export function applyEvent(gs: GameState, ev: EventType): string {
  switch (ev) {
    case "OUT":
      // OUT estocástico: posible SF o DP simplificado
      {
        const b = gs.bases;
        const outs = gs.outs;
...
// Mezcla base MLB por hit (si SLG/H no calza, reajustamos desde aquí)
...
/** Ajusta la mezcla para que el promedio de bases por hit sea ~ targetB (≥1 y ≤~2.6). */
...
function computeEventProbsFromSlash(
  ...
  const BB_raw = Math.max(0, OBP_eff - H_raw); // "BB" implícito (no emitimos evento)
...
// Rates por aparición al plato (por-PA)
...
export type Batter = {
  id: string; // identificador único en el roster
  name: string;
 ...
 * Nota: K% se integra dentro de OUT global (aún no distinguimos tipos de OUT aquí).
 */
export function eventProbsFromRateLine(rate: RateLine): EventProbs {
  ...
  const s1_raw = Math.max(0, h - hr - d2 - d3); // 1B implícitos
  ...
  // Mapear a EventProbs (sin K explícito; OUT los incluye)
  return {
    OUT: pOUT + k2, // K% también termina en OUT del motor
    BB: bb2,
    HBP: hbp2,
    "1B": s12,
    "2B": d22,
...
/** Devuelve la RateLine adecuada para el bateador según mano del pitcher. */
export function pickRateLine(b: Batter, pitcherHand: Hand): RateLine {
  return pitcherHand === "L" ? b.rates.vsL : b.rates.vsR;
}

/** Devuelve el id del bateador actual dado el lineup y un índice. */
...
export function adjustEventProbsWithPF(
  ...
  if (hitSum > 0) {
    ...
    if (total2 > 1e-12) {
      ...
      HR = HR2 * scale;
      // mantener proporciones de S1,S2,S3 entre sí
  ...
  // 3) Recalcular OUT para mantener suma ≈1
  const OUT = clamp(1 - (BB + HBP + S1 + S2 + S3 + HR), 0, 1);
  return { OUT, BB, HBP, "1B": S1, "2B": S2, "3B": S3, HR };
}

// -------- Simulación por lineup (para juegos y Monte Carlo) --------
