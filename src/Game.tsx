...
function MiniLineChart({
  ...
  function pathOf(vals: number[]): string {
    ...
    for (let i = 0; i < vals.length; i++) {
      const v = Number.isFinite(vals[i])
        ? vals[i]
        : i > 0
        ? vals[i - 1]
        : vals.find(Number.isFinite) ?? 0;
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      {yLabel && (
        <div className="muted" style={{ fontSize: ".85em", marginTop: 4 }}>
          {yLabel}
        </div>
      )}
    </div>
